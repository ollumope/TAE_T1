...
Con aras de determinar si las preguntas seleccionadas en la dimensi&oacute;n aportan al futuro modelo de agrupamiento, se busca reducir las preguntas a aquellas que representen m&aacute;s en el an&aacutelisis, esto se realiza a trav&eacute;s del modelo de componentes principales (PCA)
...
El grupo 1 se diferencia de los dem&aacuete;s porque es el que en promedio dice tener suficiente informaci&oacute;n sobre los m&eacute;todos de planificaci&oacute;n familiar y son los que m&aacute;s planifican, el grupo 2 se identifica por ser los que en promedio en los &uacute;ltimos 30 d&iacute; se enfermaron sin necesidad de hospitalizaci&oacute;n y asistieron a la red de servicios de salud, el grupo 4 se caracteriza porque en promedio tiene menor aplicaci&oacute; en todos los indicadores de esta dimensi&oacute;, por su parte el grupo 3 es un grupo cuyo compartamiento es intermedio. 
 
```{r}
df_member_salud %>% group_by(cluster) %>% summarise(p_307=min(p_307), p_308 = min(p_308), p_324= min(p_324), p_325=min(p_325)) -> ECV_SALUD_KMEANS_MIN
...
df_member_salud %>% group_by(cluster) %>% summarise(p_307=max(p_307), p_308 = max(p_308), p_324= max(p_324), p_325=min(p_325)) -> ECV_SALUD_KMEANS_MAX
