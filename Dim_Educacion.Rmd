...
ECV <- read.csv("~/Documents/GitHub/TAE_T1/dataSet/encuesta_calidad_vida.csv", sep=";", fileEncoding = "UTF-8")
...
```{r}
c_1 <-  factoextra::fviz_contrib(prcomp(ECV_FINAL_EDU[,2:7]) , choice = "var", axes = 1, top = 6, title = "Componente-1")
c_2 <- factoextra::fviz_contrib(prcomp(ECV_FINAL_EDU[,2:7]) , choice = "var", axes = 2, top = 6, title = "Componente-2")
c_3 <- factoextra::fviz_contrib(prcomp(ECV_FINAL_EDU[,2:7]) , choice = "var", axes = 3, top = 6, title = "Componente-3")

grid.arrange(c_1,c_2,c_3)
...
El cluster 1 esta comprenido en 130 barrios dispersos en todo el valle de la aburra. Los barrios son : 


```{r}
writeLines("td, th { padding : 6px } th { background-color : brown ; color : white; border : 1px solid white; } td { color : brown ; border : 1px solid brown }", con = "mystyle.css")
dset1 <- subset(ECV_FINAL_EDU_KMEANS, cluster=="1") %>% select(barrio)
...
El Cluster numero 2, se diferencia de los demas grupos debido a que el 23% las personas que viven en estos barrios, utilizan el transporte publico para dirigirse al lugar de estudio. La segunda variable más notoria es el analfabetismo, con al rededor del 18%. Las dos variables restantes tienen un comportamiento menor.
El cluster 2 esa conformado por 3 barrios estos son:

```{r}
writeLines("td, th { padding : 6px } th { background-color : brown ; color : white; border : 1px solid white; } td { color : brown ; border : 1px solid brown }", con = "mystyle.css")
dset1 <- subset(ECV_FINAL_EDU_KMEANS, cluster=="2") %>% select(barrio)
knitr::kable(dset1, format = "html")
...
```{r}
writeLines("td, th { padding : 6px } th { background-color : brown ; color : white; border : 1px solid white; } td { color : brown ; border : 1px solid brown }", con = "mystyle.css")
dset1 <- subset(ECV_FINAL_EDU_KMEANS, cluster=="4") %>% select(barrio)
knitr::kable(dset1, format = "html")
