...
Para la dimensión de MOVILIDAD  se toman las preguntas:
...
```{r}
fviz_cluster(kmeans_model, data = ECV_MOVILIDAD_SCALE)
```


```{r}
 fviz_cluster(kmeans_model, ECV_MOVILIDAD_SCALE, ellipse.type = "norm")
```
...
```{r}
#nom_bar <- merge(barrios_med, sub_, by.x="NOMBRE", by.y="barrio_2", duplicateGeoms = T, all.x = TRUE)
#names(nom_bar)
```
