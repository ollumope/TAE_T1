...
```{r}
#df[c('p_291','p_292','p_295')] <- lapply(df[c('p_291','p_292','p_295')], as.integer)
```
...
```{r}
library(dplyr)
grp <- group_by(ECV_ALIMENTO_DEP,barrio,sum())
grp
...
```{r}
grp1 <- aggregate(ECV_ALIMENTO_DEP["p_291"], by=ECV_ALIMENTO_DEP["barrio"], FUN=sum)
grp1
```
